...
struct HIDDetectionInformation {
	...
	bool compare(hid_device_info* dev_info, size_t usages_index) {
		...
		return (vid == dev_info->vendor_id && pid == dev_info->product_id &&
				usages[usages_index].interface == dev_info->interface_number &&
				(!usages[usages_index].usage_page ||
				 usages[usages_index].usage_page == dev_info->usage_page) &&
...
static std::vector<HIDDetectionInformation> hid_devices{
	...
	{"Roccat Vulcan TKL Pro",
	 0x1E7D,
	 0x311A,
	 {{0x0001, 0xFF01, 0x0001}, {0x0001, 0xFF02, 0x0001}, {0x0003, 0xFF00, 0x0001}}}};