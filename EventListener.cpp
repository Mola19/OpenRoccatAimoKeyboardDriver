...
void EventListener::register_capslock_handler(std::function<void(CapslockEvent)> capslock_handler) {
	this->capslock_handler = capslock_handler;
}

void EventListener::unregister_capslock_handler() {
	this->capslock_handler = std::nullopt;
}
...
std::unordered_map<uint8_t, Gen1OSKeytoGen2Struct> VulcanTKLKeytoGen2 = {
	{0x00, {0x01, 0x03}}, {0x01, {0x01, 0x09}}, {0x02, {0x01, 0x12}}, {0x03, {0x01, 0x13}},
	{0x04, {0x01, 0x1A}}, {0x05, {0x01, 0x0A}}, {0x06, {0x01, 0x0B}}, {0x07, {0x01, 0x14}},
	{0x08, {0x01, 0x1B}}, {0x09, {0x01, 0x23}}, {0x0A, {0x01, 0x0C}}, {0x0B, {0x01, 0x15}},
	{0x0C, {0x01, 0x1C}}, {0x0D, {0x01, 0x25}}, {0x0E, {0x01, 0x24}}, {0x0F, {0x01, 0x16}},
	{0x10, {0x01, 0x1D}}, {0x11, {0x01, 0x1C}}, {0x12, {0x01, 0x26}}, {0x13, {0x01, 0x2E}},

	{0x1E, {0x03, 0x00}}, {0x1F, {0x03, 0x08}}, {0x20, {0x03, 0x11}}, {0x21, {0x03, 0x10}},
	{0x22, {0x03, 0x18}}, {0x23, {0x03, 0x20}}, {0x24, {0x03, 0x28}}, {0x25, {0x03, 0x29}},
	{0x26, {0x03, 0x30}}, {0x27, {0x03, 0x38}}, {0x28, {0x03, 0x40}}, {0x29, {0x03, 0x41}},
	{0x2A, {0x03, 0x49}}, {0x2B, {0x03, 0x51}}, {0x2C, {0x03, 0x59}}, {0x2D, {0x03, 0x4A}},
	{0x2E, {0x03, 0x52}}, {0x2F, {0x03, 0x5A}}, {0x30, {0x03, 0x5C}}, {0x31, {0x03, 0x5D}},
	{0x32, {0x03, 0x65}}, {0x33, {0x03, 0x6D}}, {0x34, {0x03, 0x77}},

	{0x35, {0x02, 0x00}}, {0x36, {0x02, 0x08}}, {0x37, {0x02, 0x11}}, {0x38, {0x02, 0x10}},
	{0x39, {0x02, 0x18}}, {0x3A, {0x02, 0x20}}, {0x3B, {0x02, 0x28}}, {0x3C, {0x02, 0x29}},
	{0x3D, {0x02, 0x30}}, {0x3E, {0x02, 0x38}}, {0x3F, {0x02, 0x40}}, {0x40, {0x02, 0x41}},
	{0x41, {0x02, 0x49}}, {0x42, {0x02, 0x4A}}, {0x43, {0x02, 0x51}}, {0x44, {0x02, 0x52}},
	{0x45, {0x02, 0x59}}, {0x46, {0x02, 0x5A}}, {0x47, {0x02, 0x5C}}, {0x48, {0x02, 0x5D}},
	{0x49, {0x02, 0x65}}, {0x4A, {0x02, 0x6D}}, {0x4B, {0x02, 0x77}},
	...
	while (!kill_read_thread) {
		if (gen == 2) {
			...
			switch (res[2]) {
				...
				case 0xCF:
					if (state_handler)
						state_handler.value()({.state = 1, .active = static_cast<bool>(res[3] == 2)}
						);
					break;
				case 0xF1:
					if (osfn_handler)
			...
			switch (res[2]) {
				...
				case 0x07:
					...
					if (osfn_handler) {
						if (pid == 0x2FEE) {
							if (VulcanTKLKeytoGen2.count(res[3]) == 0)
								continue;
							...
							if (Vulcan1XXKeytoGen2.count(res[3]) == 0)
								continue;
					...
					break;
				case 0x08:
					// sadly this only sends for done,
					// for started it sends the packet to the endpoint for the led packets,
					// but i doesn't seem to be receivable there, so i just have to ignore it
					if (reset_handler)
						reset_handler.value()(res[3]);
					break;
				case 0x0A:
					if (capslock_handler)
						capslock_handler.value()(
							{.is_easyshift = (res[3] == 0xFF), .released = static_cast<bool>(res[4])
							}
						);
					// there is no capslock keypress event, so this has to be simulated
					if (keypress_handler)
						keypress_handler.value()(
							{.key = 0x05, .released = static_cast<bool>(res[4])}
						);
					break;
				case 0x0B:
					if (mm_handler)
				...
				case 0x0C:
					if (lighting_handler)
						lighting_handler.value()(
							{.type = 1, .value = res[3], .positive_change = std::nullopt}
						);
					break;
				case 0x0D:
					// start timer
				...
				case 0xCC:
					if (wheel_handler)
						wheel_handler.value()(
							{.clockwise = static_cast<bool>(res[3] == 1), .volume_mode = !res[4]}
						);
					break;
				case 0xCE:
					if (lighting_handler)
